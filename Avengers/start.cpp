...
int main() {
    ...
    Display* display = Display::instance();//access the display class
    erase();
    display->print("Avengers", 15, 55);//the intro screen
    refresh();
    usleep(300000);

    while (true){ // loop for the whole game to run
	...
	// initialization of the maps and their pilse
    ...
    //the menu intro
        ...
        while (true)//the loop for the whole game
        {
    	erase();

	display->print("Avengers", 1, 50); // screen for selecting to play game, know about the game background, or exit the game
	display->print("start", 14, 60);
	display->print("about", 23, 60);
	display->print("exit", 32, 60);
	display->print("arrow", 5 + (9 * choice), 45); // arrow to control the action to be executed
	refresh();
	usleep(300000);

	inp = getch();// input from user for the movement of arrow
	...
	if (inp == 'q')
	{
		choice = 3; // if the user selects the q it will move him to move out and quit
		break;
	}
	}
	    
    if (choice == 3) // represents quiting the game
    {
    	break;
    }
    else if (choice == 2) // the about of the game
    {
	//about
	erase();
	display->print("Storyline", 3, 15); // display the storyline
	...
	continue;// to skip the game
    }

    choice = 1;
    while (true){ //selecting the difficulty of the game
        erase();
	
    	display->print("Choose a Level",5,15); //choosing the difficulty
    	display->print("easy", 14, 50);
    	display->print("medium", 23, 50);
    	display->print("hard", 32, 50);

    	display->print("arrow", 5 + (9 * choice), 35);//arrow printing where the user has selected
	refresh();
	
	usleep(300000);
	inp = getch();//controlling the arrow using the user input
	if (inp == KEY_UP)
		choice -= 1;
	...
	if (choice < 1)// to make sure the choice is between 1 and 3 and user does not go out of bound
		choice = 1;
	else if (choice > 3)
		choice = 3;
	if (inp == 10 || inp == KEY_ENTER)//to select the difficulty
		break;
    }
    ...
    while (true)//selecting the level of the game
    {
	erase();
	display->print("Choose your map", 2, 15); // printing the screen
	...
	if (level < 6)
	{
		display->print("arrow", 4 + (level * 6) ,0); // printing arrow based on user selection
	}
	else
	...
	if (inp == KEY_UP && level != 1 && level != 6 ) // making the arrow movement
		level-= 1;
    ...
    display->selectCharacter();//selecting characters using the function in the class

    if (text == 1) {//training AI according to levels and difficulty and playing the game too
        if (level == 1) {
    ...
    } else {//displaying error in code
        display->error("Invalid input!");
    }
    
    erase();
    display->print("Game Ended", 15, 40);//printing the game has ended
    refresh();
