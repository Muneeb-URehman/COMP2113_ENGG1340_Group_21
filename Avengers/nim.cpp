...
public:
    ...
    int get_player() const { return player; }    // Getters and Setters
...
public:
    ...
    std::pair<int, int> choose_action(const std::vector<int>& state,
        bool use_epsilon = true) {     // To find the best possible action. If no epsilon, returns highest reward or else chooses uniformly.
...
void play(NimAI& ai, const std::vector<int>& map) {   //Start game
    Nim game(map);
